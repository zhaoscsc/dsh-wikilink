...
早期版本需要对已安装的 `@deepseek-ai` 客户端 bundle 打补丁（harness 输入触发管线只识别 `/` 和 `@`）。v0.2 起选择器完全自绘：插件自己监听草稿、在公开的 `conversation.input.overlay` 插槽渲染浮层菜单、通过公开输入动作写回闭合的 `[[标题]]`。**零补丁，无需重打——安装、重启、即用。** 历史补丁脚本 `apply-harness-patches.mjs` 仅作存档保留。
...
- 选择器只在草稿**末尾**存在未闭合的 `[[`/`【【` 时触发——公开输入状态不含光标信息，光标不在末尾时不触发
